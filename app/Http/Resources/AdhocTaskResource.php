...
class AdhocTaskResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'requires_cleanup' => (bool) $this->requires_cleanup,
            ...
            'status' => $this->status,
            'stage' => $this->stage ?? 'pending',
            'has_foto_bukti' => !empty($this->foto_bukti),
            'foto_bukti_url' => !empty($this->foto_bukti) ? url("/api/v1/adhoc-tasks/{$this->id}/foto-bukti") : null,
            'has_foto_bukti_persiapan' => !empty($this->foto_bukti),
            'foto_bukti_persiapan_url' => !empty($this->foto_bukti) ? url("/api/v1/adhoc-tasks/{$this->id}/foto-persiapan") : null,
            'has_foto_bukti_cleanup' => !empty($this->foto_bukti_cleanup),
            'foto_bukti_cleanup_url' => !empty($this->foto_bukti_cleanup) ? url("/api/v1/adhoc-tasks/{$this->id}/foto-cleanup") : null,
            'verification_notes' => $this->verification_notes,
            'started_at' => $this->started_at?->toIso8601String(),
            'submitted_at' => $this->submitted_at?->toIso8601String(),
            'setup_submitted_at' => $this->setup_submitted_at?->toIso8601String(),
            'cleanup_submitted_at' => $this->cleanup_submitted_at?->toIso8601String(),
